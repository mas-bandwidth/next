...
func (stream *WriteStream) SetError(err error) {
...
func (stream *WriteStream) SerializeInteger(value *int32, min int32, max int32) {
	...
	if *value < min {
		stream.SetError(fmt.Errorf("value (%d) should be at least min (%d)", *value, min))
		return
	}
	if *value > max {
		stream.SetError(fmt.Errorf("value (%d) should be no more than max (%d)", *value, max))
		return
	}
	bits := BitsRequired(uint32(min), uint32(max))
	unsignedValue := uint32(*value - min)
	stream.SetError(stream.writer.WriteBits(unsignedValue, bits))
}
...
func (stream *WriteStream) SerializeBits(value *uint32, bits int) {
	...
	if bits < 0 || bits > 32 {
		stream.SetError(fmt.Errorf("bits (%d) should be between 0 and 32", bits))
		return
	}
	stream.SetError(stream.writer.WriteBits(*value, bits))
}
...
func (stream *WriteStream) SerializeBool(value *bool) {
	...
	stream.SetError(stream.writer.WriteBits(uint32Value, 1))
}
...
func (stream *WriteStream) SerializeFloat32(value *float32) {
	...
	stream.SetError(stream.writer.WriteBits(math.Float32bits(*value), 32))
}
...
func (stream *WriteStream) SerializeBytes(data []byte) {
	...
	if len(data) == 0 {
		stream.SetError(fmt.Errorf("byte buffer should have more than 0 bytes"))
		return
	}
	stream.SerializeAlign()
	stream.SetError(stream.writer.WriteBytes(data))
}
...
func (stream *WriteStream) SerializeString(value *string, maxSize int) {
	...
	if length > int32(maxSize) {
		stream.SetError(fmt.Errorf("string is longer than maxSize"))
		return
	}
	min := int32(0)
	max := int32(maxSize) - 1
...
func (stream *WriteStream) SerializeAlign() {
	...
	stream.SetError(stream.writer.WriteAlign())
}
...
func (stream *WriteStream) Flush() {
	...
	stream.SetError(stream.writer.FlushBits())
}
