...
func (env *TestEnvironment) GetBestRoutes(routeMatrix []core.RouteEntry, sourceRelays []string, sourceRelayCost []int32, destRelays []string, maxCost int32) []TestRouteData {
	...
	core.GetBestRoutes(routeMatrix, sourceRelayIndex, sourceRelayCost, destRelayIndex, maxCost, bestRoutes, &numBestRoutes)
...
func (env *TestEnvironment) GetBestRoute_Initial(routeMatrix []core.RouteEntry, sourceRelayNames []string, sourceRelayCost []int32, destRelayNames []string, maxCost int32) (int32, []string) {
	...
	debug := ""
	selectThreshold := int32(2)
	hasRoute := core.GetBestRoute_Initial(routeMatrix, sourceRelays, sourceRelayCost, destRelays, maxCost, selectThreshold, &bestRouteCost, &bestRouteNumRelays, &bestRouteRelays, &debug)
	if !hasRoute {
		return 0, []string{}
	}
...
func TestTheTestEnvironment(t *testing.T) {
	...
	routeMatrix := core.Optimize(numRelays, numSegments, costMatrix, env.price, relayDatacenters, nil)
...
func TestOptimizeRouteCosts(t *testing.T) {
	...
	routeMatrix := core.Optimize(numRelays, numSegments, costMatrix, relayPrice, relayDatacenters, nil)

	checkRouteCosts(t, routeMatrix, numRelays, costMatrix)
}

func TestOptimizeDestRelaysRouteCosts(t *testing.T) {
	...
	routeMatrix := core.Optimize(numRelays, numSegments, costMatrix, relayPrice, relayDatacenters, destinationRelay)
...
func NewTestData(env *TestEnvironment) *TestData {
	...
	test.routeMatrix = core.Optimize(test.numRelays, numSegments, test.costMatrix, env.price, test.relayDatacenters, nil)
