...
- **Advanced packet filter is ON everywhere** (`NEXT_ADVANCED_PACKET_FILTER=1` in sdk/include/next.h,
  ...
  in the Build XDP job pins Go vs XDP every tag). magic_backend mixes an optional per-install
  `MAGIC_KEY` into the magic derivation (added 2026-07-13; base64 32 bytes in
  `~/secrets/<env>-magic-key.txt`, generated by `next keygen`, back-filled by `next config`,
  passed to magic_backend only via terraform in all three envs). All magic_backend instances
  in an env MUST share the same key. Empty = deterministic constants-only derivation — that
  is deliberate for local dev and functional tests; never commit a magic key.
